...
void rtinit1() {
	...
	neighbor1 = getNeighborCosts(NODEID); // 构造节点
	numNodes = neighbor1->NodesInNetwork; // 节点数量

	// initialize distance table
	for (i = 0; i < numNodes; i++) {
		for (j = 0; j < numNodes; j++) {
			if (i == j) { // directly connected neighbor
				dt1.costs[i][j] = neighbor1->NodeCosts[i]; // i==j表示经过j到达i，即有一条直接通路
			}
			else {
	...
	// 广播给其他节点
	for (i = 0; i < numNodes; i++) {
		if (i != NODEID && neighbor1->NodeCosts[i] < INFINITY) { // 表明这是其邻居节点
			if (TraceLevel >= 3) printf("making packet %d...\n", i);
			toSend.sourceid = NODEID;		// 设置源节点
			toSend.destid = i;				// 设置目的节点
			for (j = 0; j < numNodes; j++) {
				toSend.mincost[j] = dt1.costs[j][j];		// 当前到其他节点的最小代价即直接经过j到j
			}
...
void rtupdate1( struct RoutePacket *rcvdpkt ) {
	...
	for (i = 0; i < numNodes; i++) {
		if (dt1.costs[i][src] > dt1.costs[src][src] + rcvdpkt->mincost[i]) {	// 如果通过该点能够更快到达i
			dt1.costs[i][src] = dt1.costs[src][src] + rcvdpkt->mincost[i];		// 那么根据该点更新到i的距离
			if (i != NODEID) {
		...
		for (i = 0; i < numNodes; i++) {
			for (j = 0; j < numNodes; j++) {
				if (toSend.mincost[i] > dt1.costs[i][j])
					toSend.mincost[i] = dt1.costs[i][j];	// 遍历图，更新mincost数组
			}
		}
