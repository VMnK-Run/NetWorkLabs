...
main(int argc, char* argv[]){
    if (argc != 7){
        fprintf(stderr, "usage: %s <msg_num> <loss> <corruption> <snd_time> <trace> <runi>\n",argv[0]);
        return;
    }
	nsimmax = atoi(argv[1]);
...
init()                         /* initialize the simulator */
{
    ...
    printf("Enter the number of messages to simulate: %d", nsimmax);
	//scanf("%d",&nsimmax);
	printf("Enter  packet loss probability [enter 0.0 for no loss]: %f", lossprob);
	// scanf("%f",&lossprob);
	printf("Enter packet corruption probability [0.0 for no corruption]: %f", corruptprob);
	// scanf("%f",&corruptprob);
	printf("Enter average time between messages from sender's layer5 [ > 0.0]: %f", lambda);
	// scanf("%f",&lambda);
	printf("Enter TRACE: %d", TRACE);
	// scanf("%d",&TRACE);
...
int AorB;  /* A or B is trying to stop timer */
{
    ...
    for (q=evlist; q!=NULL ; q=q->next){
        ...
        if ( (q->evtype==TIMER_INTERRUPT  && q->eventity==AorB) ) { 
            ...
            else if (q->next==NULL) q->prev->next = NULL;     /* end of list - there is one in front */
